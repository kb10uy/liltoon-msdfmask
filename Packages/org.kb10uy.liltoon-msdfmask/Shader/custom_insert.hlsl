        ...
        if(_UseShadow)
        {
            ...
            float shadowStrengthMask = 1;
                ...
                // MSDF Mask Process
                    ...
                    float4 sampledStrength = LIL_SAMPLE_2D(_ShadowStrengthMask, lil_sampler_linear_repeat, fd.uvMain);
                    if(_ShadowStrengthMaskLOD) sampledStrength = LIL_SAMPLE_2D_GRAD(_ShadowStrengthMask, lil_sampler_linear_repeat, fd.uvMain, max(fd.ddxMain, _ShadowStrengthMaskLOD), max(fd.ddyMain, _ShadowStrengthMaskLOD));
                #else
                    float4 sampledStrength = LIL_SAMPLE_2D_GRAD(_ShadowStrengthMask, lil_sampler_linear_repeat, fd.uvMain, max(fd.ddxMain, _ShadowStrengthMaskLOD), max(fd.ddyMain, _ShadowStrengthMaskLOD));
                #endif
                float strengthValue = sampledStrength.r;
                if (_CustomUseMsdfMaskShadow == 1) strengthValue = lilMSDF(sampledStrength.rgb) * sampledStrength.a;
                if (_CustomUseMsdfMaskShadow == 2) strengthValue = 1.0 - lilMSDF(sampledStrength.rgb) * sampledStrength.a;
                shadowStrength *= strengthValue;
            #endif
            if(_ShadowMaskType)
            {
